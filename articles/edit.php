...
if (!$article) {
    set_flash_message('error', 'Article not found or you do not have permission to edit it');
    redirect('/articles/');
}

$title = 'Edit Article - Nayash Blog';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            ...
            $errors['general'] = 'Failed to update article. Please try again.';
        }
    }
}
    ...
    <form method="POST" class="bg-white rounded-lg shadow-md p-6">
            ...
            <select id="category_id" name="category_id" 
                ...
                <?php foreach ($categories as $category): ?>
                    <option value="<?= $category['id'] ?>" 
                        <?= (isset($_POST['category_id']) ? $_POST['category_id'] : $article['category_id']) == $category['id'] ? 'selected' : '' ?>>
        ...
        <div class="mb-6">
            ...
            <textarea id="content" name="content" rows="10"
                      class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"><?= htmlspecialchars($_POST['content'] ?? $article['content']) ?></textarea>
